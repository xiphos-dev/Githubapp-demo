...
class GithubRepo():
    ...
    async def download_files_decision(self, token, owner, repo, branch):
        ...
        if  len(folder_paths) > 5000: #Github API request limit per hour
            ...
            destination_path = "./destination" #  Change folder to real path
    ...
    async def fetch_files_in_parallel(self, token, owner, repo, branch, folder_paths, max_concurrent_requests=10):
        # Function to fetch file contents in parallel from multiple folders using graphql
    ...
    async def fetch_files_from_folder(self, semaphore, session, token, owner, repo, branch, folder_path):
        #Downloads blob metadata from an entire repository folder, Ignores subfolders.
        max_retries = 5
    ...
    def get_files_from_branch(self,token, owner, repo, branch):
        # Returns metadata for all files and folders inside repostory. Does not download file contents.
...
async def main():
    ...
    #### Public repository with over 30000 files
    #repo="QGIS"
    #owner="qgis"
    #branch = "master"
    
    
    ### Small private repository for testing, less than 20 files
    repo="Githubapp-demo"
    owner="xiphos-dev"
    branch = "main"
